...
const icon = d => {
  ...
  if (d.id > 100000) { // not native
    return Promise.resolve('download.png');
  }
  else {
    return chrome.downloads.getFileIcon(d.id, {
      size: 32
    }).then(href => {
      icon.cache[d.mime] = href || 'download.png';

      return icon.cache[d.mime];
    });
  }
};
icon.cache = {};
