/**
    ...
    Copyright (C) 2014-2026 [InBasic](https://webextension.org/listing/turbo-download-manager-v2.html)
...
class File { /* write to disk */
  ...
  store({url, filename}) {
    return new Promise((resolve, reject) => {
      const port = chrome.runtime.connect({name: 'file.store'});
  ...
  async download(options, started = () => {}) {
    if (options.dialog) {
      ...
      const code = await new Promise(resolve => {
        ...
        chrome.runtime.sendMessage({
          method: 'file.window',
          args: {
            url: '/downloads/save-dialog/index.html' +
    ...
    return new Promise((resolve, reject) => {
      const port = chrome.runtime.connect({name: 'file.store.watch'});
      port.onMessage.addListener(request => {
        if (request.method === 'started') {
          started(request.d);
        }
